...
void updateNeighbourList (particle *particles, double boxMatrix[2][2]) {
    ...
    for (int i=0;i<activeN-1;i++) {
        double foundNeighboursR2[100];
        int foundNeighboursIndex[100],licznik=0;
        for (int j=i+1;j<activeN;j++) {
            ...
            if (r2<neighRadius2) { //znalezienie wszystkich sąsiadów wewnątrz okręgu o R=neighRadius, żeby nie szukać najbliższych wśród WSZYSTKICH
                foundNeighboursIndex[licznik]=j;
                foundNeighboursR2[licznik++]=r2;
            }
        }
        while (licznik>6) { //wyrzucenie najdalszych nadmiarowych sąsiadów
            int maxIndex=0;
            for (int j=1;j<licznik;j++) maxIndex=foundNeighboursR2[j]>foundNeighboursR2[maxIndex]?j:maxIndex;
            for (int j=maxIndex;j<licznik-1;j++) {
                foundNeighboursIndex[j]=foundNeighboursIndex[j+1];
                foundNeighboursR2[j]=foundNeighboursR2[j+1];
            }
            licznik--;
        }
        for (int j=0;j<licznik;j++) {
            particles[i].neighbours[particles[i].neighCounter++]=foundNeighboursIndex[j];
            particles[foundNeighboursIndex[j]].neighbours[particles[foundNeighboursIndex[j]].neighCounter++]=i;
        } //neighCounter będzie dla i=0 równy 6, a dla wyższych WIĘKSZY od 6 (zaliczani będą zawsze 'dodatkowi najbliżsi 6' oprócz dodanych wcześniej [z iteracji wcześniejszych cząstek])
    }
}
...
void scaleParticlesPositions (particle *particles, double boxMatrix[2][2]) { //UWAGA: nie sprawdza się tu przekryć - działa tylko dla rozprężania (HCH są gwiaździste [wypukłe nie są], czyli istnieje punkt [po środku], z którego można poprowadzić linię wewnątrz ciała do każdego jego punktu, więc tak można [bez sprawdzania przekryć])
    for (int i=0;i<activeN;i++) {
        particles[i].r[0]=boxMatrix[0][0]*particles[i].normR[0]+boxMatrix[0][1]*particles[i].normR[1];
        particles[i].r[1]=boxMatrix[1][0]*particles[i].normR[0]+boxMatrix[1][1]*particles[i].normR[1];
    }
}
/////////////////  } PARTICLE functions
...
double getPacFracFromEOS (double p) {
    if (p<3.1)
        return 2.9653746095434106-3.5798873981878625*p+3.524800308885225*p*p-1.9705876588786462*p*p*p+0.6328389618067031*p*p*p*p-0.1089763495313435*p*p*p*p*p+0.007796962582295849*p*p*p*p*p*p;
    else if (p<3.3)
        return -60343.39602329992+113814.03131068758*p-89426.50421754201*p*p+37468.0632439155*p*p*p-8828.884512335213*p*p*p*p+1109.3725203012848*p*p*p*p*p-58.07207055029417*p*p*p*p*p*p;
    else if (p<3.5)
        return -2993288.049814932+5324998.295824717*p-3946225.1484792405*p*p+1559356.158567375*p*p*p-346523.8926764994*p*p*p*p+41060.21550865297*p*p*p*p*p-2026.7448975829575*p*p*p*p*p*p;
    else
        return 229.74246563711458-325.1927147820305*p+192.45414493393676*p*p-60.61082579287017*p*p*p+10.708121483395587*p*p*p*p-1.0058550213049378*p*p*p*p*p+0.03923431425024388*p*p*p*p*p*p;
}
...
/////////////////////////////////////////////// WARUNKI POCZATKOWE
    ...
    double arg[2]={startArg[0],startArg[1]};
    while (growing>=0) {
        double pressureReduced=arg[0],
        ...
        for (int i=0;i<2;i++) boxMatrix[i][i]=matrixOfParticlesSize[i]*unitCellAtCP[i]/(double)n[i]*NLinearMod*sqrt(getPacFracFromEOS(pressureReduced));
        boxMatrix[1][0]=0.0; boxMatrix[0][1]=0.0;
        if (startArg[0]!=arg[0]) scaleParticlesPositions(particles,boxMatrix);
...
/////////////////////////////////////////////// RDZEN MC
            ...
            int cycleCounter=0, indexScanned=(matrixOfParticlesSize[0]*round(matrixOfParticlesSize[1]*NLinearMod/2.0)-round(matrixOfParticlesSize[0]/2.0))*NLinearMod;
            ...
            for (double iStep=1;iStep<nStep;iStep++) {
                int randIndex = (int)(MTGenerate(randomStartStep)%1000000/1000000.0*activeN);
                attemptedNumberR++;
                    ...
                    if (cycle%intervalSampling==0) {
                        ...
                        if (cycle>cyclesOfEquilibration) {
                            ...
                            if (cycle%intervalResults==0)
                                fprintf(fileAllResults,"%ld\t%.17f\t%.17f\t%.17f\t%.17f\t%.17f\t%.17f\t\n",(cycle+(long)args[4]),volume,boxMatrix[0][0],boxMatrix[1][1],boxMatrix[0][1],rho,pacFrac);
                            ...
                            if (cycle%intervalOutput==0) {
                                ...
                                printf("   AccRatR: %.4f, dR: %.4f, dV: %.4f\n",acceptanceRatioR,deltaR,deltaV);
                                printf("   Dens: %.4f, V/V_cp: %.4f, PressRed: %.4f\n",rho,pacFrac,pressureReduced);
                                printf("   box00: %.8f, box11: %.8f, box01(10): %.8f\n",boxMatrix[0][0],boxMatrix[1][1],boxMatrix[0][1]);
