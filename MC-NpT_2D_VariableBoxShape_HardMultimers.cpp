...
int attemptToDisplaceAParticle (particle *particles, int index, double boxMatrix[2][2], double randomValue) {
    int result=1;
        ...
        if (!onlyMath[0]) {
            if (arg[0]==startArg[0] && !loadedConfiguration) {
                ...
                while (dataType<12) {
                    ...
                    if (dataType<11) { //stage #1 configuration parameters
                        if (character==' ') {
                            ...
                            if (dataType==11) {
                                ...
                                deltaR=args[8]; deltaPhi=args[9]; deltaV=args[10];
...
/////////////////////////////////////////////// RDZEN MC
            ...
            for (double iStep=1;iStep<nStep;iStep++) {
                ...
                if (randIndex<activeN) {
                    double randomTransOrientat = MTGenerate(randomStartStep)%1000000/1000000.0;
                    if (randomTransOrientat<0.5) {
                        attemptedNumberR++;
                        if (attemptToDisplaceAParticle(particles,randIndex,boxMatrix,randomTransOrientat))
                            displacedNumberR++;
                    } else {
                        attemptedNumberPhi++;
                        if (attemptToDisplaceAParticle(particles,randIndex,boxMatrix,randomTransOrientat))
                ...
                if (cycleCounter>=fullCycle) {
                    ...
                    if (cycle%intervalSampling==0) {
                        ...
                        /*if (cycle%50==0) {
                            ...
                            printf("   AccRatR: %.4f, dR: %.4f, AccRatPhi: %.4f, dPhi: %.4f, AccRatV: %.4f, dV: %.4f\n",acceptanceRatioR,deltaR,acceptanceRatioPhi,deltaPhi,acceptanceRatioV,deltaV);
                        ...
                        if (cycle>cyclesOfEquilibration) {
                            ...
                            if (cycle%intervalOutput==0) {
                                ...
                                fprintf(fileConfigurations,"%.1f %.1f %.17f %.17f %ld %.17f %.17f %.17f %.17f %.17f %.17f {",randomStartStep[0],randomStartStep[1],rho,pressureReduced,(cycle+(long)args[4]),boxMatrix[0][0],boxMatrix[1][1],boxMatrix[0][1],deltaR,deltaPhi,deltaV);
                                for (int i=0;i<activeN;i++)
                            ...
                            if (acceptanceRatioPhi>desiredAcceptanceRatioR) deltaPhi*=1.05; else deltaPhi*=0.95;
                            if (deltaPhi>maxDeltaR*2.0*sin(C)) deltaPhi=maxDeltaR*2.0*sin(C);
