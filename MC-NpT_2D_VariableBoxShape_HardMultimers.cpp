...
int N,gaps,activeN,loadedConfiguration,loadType=0,loadedSetStartGenerator,loadedSetGenerator,iterationsNumber,
    growing,multimerN,sideFixed,countCollidingPairs,ODFLength,skipFirstIteration,
...
char buffer[200]="",bufferN[20],bufferGaps[20],bufferG[5],bufferMN[20],bufferMS[20],bufferMD[20],bufferP[20],bufferFolderIndex[5],
...
int attemptToChangeVolume (particle *particles, double pressure, double boxMatrix[2][2], double *volume) {
    ...
    if (pressure>pressureOfNotFluid) {  //dozwolone zmiany ksztaltu pudla (faza stala)
        ...
        if (sideFixed) {
            newBoxMatrix[0][0]=boxMatrix[0][0];
            lnNewBoxMatrix[1][1]+=(MTGenerate(randomStartStep)%1000000/1000000.0-0.5)*deltaV;
            newBoxMatrix[1][1]=exp(lnNewBoxMatrix[1][1]);
        } else {
            newBoxMatrix[1][1]=boxMatrix[1][1];
...
/////////////////////////////////////////////// DANE WEJSCIOWE
    int testValue; do {
        ...
        while(fgets(config,300,fileConfig)!=NULL) {
            ...
            switch (dataIndex) {
                ...
                case 6:sideFixed=strtol(data,NULL,10);break;
    ...
    char JOBID[50]="j-"; int pointNumber=0, pointNumberP=0;
    if (argumentsNumber==1) {
        ...
        switch (strtol(arguments[1],NULL,10)) {
            ...
            case 2: //ustaw JOBID, run z najistotniejszymi parametrami z 'config.txt' nadpisanymi z poziomu wywolania
                if (argumentsNumber==12) {
                    ...
                    pointNumberP=strtol(arguments[11],NULL,10);
                } else correctNumberOfArguments=0; break;
            ...
            case 4: //ustaw JOBID, tryb loadowany #2 od zadanego numeru punktu (0->startArg) w odpowiednim folderze i trybie
                if (argumentsNumber==13) {
                    ...
                    useSpecificDirectory=strtol(arguments[10],NULL,10);
                    skipFirstIteration=strtol(arguments[11],NULL,10);
                    pointNumberP=strtol(arguments[12],NULL,10);
                } else correctNumberOfArguments=0; break;
        ...
        if (!correctNumberOfArguments) {
            ...
            printf("If type of run is '2', next arguments: $JOBID, N, gaps, multimerD, growing, iterationsNumber, useSpecificDirectory, skipFirstIteration, pointNumber, pointNumberP\n");
            ...
            printf("If type of run is '4', next arguments: $JOBID, JOBID of configuration to load, N, gaps, multimerD, growing, pointNumber, iterationsNumber, useSpecificDirectory, skipFirstIteration, pointNumberP\n");
            printf("If type of run is '5', next arguments: $JOBID, lines to skip from Results, N, gaps, multimerD, growing, pointNumber, iterationsNumber, useSpecificDirectory, pointNumberP\n");
            return 0;
        }
    }
    ...
    if (useFileToIterate) {
        if (growing) {
            ...
            startArg=iterationTable[pointNumberP][1];
        } else {
            startMaxPacFrac=iterationTable[fileIterateIterationsNumber-1][0];
            startArg=iterationTable[fileIterateIterationsNumber-1-pointNumberP][1];
        }
    ...
    sprintf(bufferMN,"%d",multimerN); sprintf(bufferMS,"%.2f",multimerS); sprintf(bufferMD,"%.6f",multimerD); sprintf(bufferP,"%.3f",startArg);
    ...
    fileResults = fopen(resultsFileName,"rt"); if (fileResults==NULL) {
        ...
        fprintf(fileResults,"Cycles\tPressure\tVolume\tl*\tBoxMatrix[0][0]\tBoxMatrix[1][1]\tBoxMatrix[1][0]([0][1])\tRho\tV/V_cp\tnu_L_yx\tnu_L_xy\tnu_eps_yx\tnu_eps_xy\tavNu_L\tavNu_eps\tODFMax_One\t<cos(6Phi)>_One\tODFMax_All\t<cos(6Phi)>_All\n");
        fclose(fileResults);
    }
    ...
    while (growing>=0) {
        ...
        if (startArg==arg) {
            if (N==56 || N==224 || N==504 || N==896) {
                ...
                double L0_coefficient=sqrt(N*VcpPerParticle/(28*sqrt(3)));  //musi byc G=0 (startMaxPacFrac)
                if (sideFixed) {
                    boxMatrix[0][0]=sqrt(startMaxPacFrac)*7.0*L0_coefficient;
                    boxMatrix[1][1]=volume/boxMatrix[0][0];
                } else {
                    boxMatrix[1][1]=sqrt(startMaxPacFrac)*8*sqrt(3/4.0)*L0_coefficient;
                    boxMatrix[0][0]=volume/boxMatrix[1][1];
                }
...
/////////////////////////////////////////////// RDZEN MC
            ...
            char allResultsFileName[200],bufferConfigurations[200],bufferOrientations[200],allOrientationsFileName[200],bufferOrientationsResults[200],allOrientationsResultsFileName[200],bufferSPF[100];
            ...
            sprintf(bufferSPF,"%.6f",startMaxPacFrac);
            strncat(allResultsFileName,"_spf-",6); strncat(allResultsFileName,bufferSPF,100); strncat(allResultsFileName,"_Results.txt",13);
            strncat(bufferConfigurations,"_spf-",6); strncat(bufferConfigurations,bufferSPF,100); strncat(bufferConfigurations,".txt",5);
            strncat(bufferOrientations,"_spf-",6); strncat(bufferOrientations,bufferSPF,100); strncat(bufferOrientations,".txt",5);
            strncat(allOrientationsFileName,"_spf-",6); strncat(allOrientationsFileName,bufferSPF,100); strncat(allOrientationsFileName,"_allOnt.txt",12);
            strncat(bufferOrientationsResults,"_spf-",6); strncat(bufferOrientationsResults,bufferSPF,100); strncat(bufferOrientationsResults,".txt",5);
            strncat(allOrientationsResultsFileName,"_spf-",6); strncat(allOrientationsResultsFileName,bufferSPF,100); strncat(allOrientationsResultsFileName,"_allOnt.txt",12);
...
/////////////////////////////////////////////// OBLICZENIE WYNIKOW
            ...
            double nu_L_yx=0, nu_L_xy=0, nu_eps_yx=0, nu_eps_xy=0;
            fileAllResults=fopen(allResultsFileName,"rt");
            if (onlyMath[0]) for (int i=0;i<onlyMath[1];i++) fgets(linia,300,fileAllResults);
            while(fgets(linia,300,fileAllResults)!=NULL) {
                sscanf(linia,"%c",linia);
            ...
            double avNu_L=(nu_L_yx+nu_L_xy)/2.0, avNu_eps=(nu_eps_yx+nu_eps_xy)/2.0;
            printf("done\n");
...
/////////////////////////////////////////////// ZAPIS DANYCH DO PLIKU
            ...
            fprintf(fileResults,"%ld\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\n",(cycle+arg5),pressure,avVolume,sqrt(startMaxPacFrac),avBoxMatrix[0],avBoxMatrix[1],avBoxMatrix[2],avRho,avPacFrac,nu_L_yx,nu_L_xy,nu_eps_yx,nu_eps_xy,avNu_L,avNu_eps,ODFMaxOne,averageCos6PhiOne,ODFMaxAll,averageCos6PhiAll);
