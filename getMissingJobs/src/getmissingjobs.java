        ...
        try {
            ...
            while ((buffer=runningJobs.readLine())!=null) {
                ...
                for (int i=0;i<result.length;i++) if (result[i].contains(filterText)) {
                    ...
                    jobFailedList[parametersValues[2]][parametersValues[0]-1][parametersValues[1]]=false;
                    break;
                }
            }
            runningJobs.close();
            ...
            for (int l=0;l<licznik;l++) for (int i=minDirectory-1;i<jobFailedList[0].length;i++) for (int j=minPoint;j<jobFailedList[0][0].length;j++) if (jobFailedList[l][i][j]) {
                ...
                String actualFolderList[] = new File("2D_N-"+N+"_gaps-"+gaps+"_G-"+G+"_badanie-"+String.valueOf(i+minDirectory)+"_mN-"+mN+"_mD-"+mD+"_p-"+startArgumentsList[G.equals("1")?l:(licznik-1-l)][1]).list();
